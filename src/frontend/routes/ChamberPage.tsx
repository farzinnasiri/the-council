...
export function ChamberPage() {
  ...
  const setChamberTimeAwareReentryEnabled = useAppStore((state) => state.setChamberTimeAwareReentryEnabled);
  const markChamberTimeAwareReentryNoticeSeen = useAppStore((state) => state.markChamberTimeAwareReentryNoticeSeen);
  const dismissChamberTimeAwareReentryNotice = useAppStore((state) => state.dismissChamberTimeAwareReentryNotice);
  ...
  const reentryNoticePendingByConversation = useAppStore((state) => state.timeAwareReentryNoticePendingByConversation);
  ...
  const showReentryNotice =
    Boolean(
      reentryNoticePendingByConversation[conversation.id] ||
      (conversation.timeAwareReentryState && !conversation.timeAwareReentryNoticeSeenAt)
    );
    ...
    <div className="mx-auto flex w-full max-w-4xl items-start justify-between gap-3 rounded-lg border border-border bg-card px-4 py-3">
      ...
      <div className="flex shrink-0 items-center gap-2">
        <Button
          ...
          onClick={() => {
            void setChamberTimeAwareReentryEnabled(conversation.id, false)
              .then(() => markChamberTimeAwareReentryNoticeSeen(conversation.id))
              .finally(() => dismissChamberTimeAwareReentryNotice(conversation.id));
          }}
        >
        ...
        <Button
          ...
          className="h-8 px-2 text-xs"
          onClick={() => {
            void markChamberTimeAwareReentryNoticeSeen(conversation.id)
              .finally(() => dismissChamberTimeAwareReentryNotice(conversation.id));
          }}
        >
          Dismiss
  ...
  return (
    <ChatScreen
      ...
      onTimeAwareReentryEnabledChange={(enabled) => setChamberTimeAwareReentryEnabled(conversation.id, enabled)}
      emptyState={{
      ...
      onSend={async ({ text }) => {
        ...
        await generateReplies(conversation.id, text, [], sendResult?.previousActiveMessageAt);
      }}
    />
  );
}
