...
import { resolveConvexUrl } from "../lib/resolveConvexUrl";
...
function toMember(doc: ConvexMemberDoc): Member {
  return {
    ...
    ttsVoiceName:
      (doc.ttsVoiceName as MemberVoiceName | undefined) ?? DEFAULT_MEMBER_VOICE,
    ...
    personalSourcesPermissionEnabled: Boolean(
      doc.personalSourcesPermissionEnabled,
    ),
...
function toConversation(doc: ConvexConversationDoc): Conversation {
  return {
    ...
    hallMode:
      doc.kind === "hall"
        ? ((doc.hallMode as "advisory" | "roundtable" | undefined) ??
          "advisory")
        : undefined,
    chamberResponseMode:
      doc.kind === "chamber"
        ? ((doc.chamberResponseMode as ChamberResponseMode | undefined) ??
          "instant")
        : undefined,
    timeAwareReentryEnabled:
      doc.kind === "chamber"
        ? Boolean(doc.timeAwareReentryEnabled ?? true)
        : undefined,
    personalSourcesEnabled:
      doc.kind === "chamber"
        ? Boolean(doc.personalSourcesEnabled ?? true)
        : undefined,
    timeAwareReentryState:
      doc.kind === "chamber" && doc.timeAwareReentryState
        ? {
            gapBucket: doc.timeAwareReentryState
              .gapBucket as TimeAwareReentryGapBucket,
            repliesRemaining: doc.timeAwareReentryState.repliesRemaining as
              | 1
              | 2,
            explicitContinuation: Boolean(
              doc.timeAwareReentryState.explicitContinuation,
            ),
            activatedAt: doc.timeAwareReentryState.activatedAt,
          }
        : undefined,
    timeAwareReentryNoticeSeenAt:
      doc.kind === "chamber" ? doc.timeAwareReentryNoticeSeenAt : undefined,
    guidanceLastReflectedUserTurnCount:
      doc.kind === "chamber"
        ? doc.guidanceLastReflectedUserTurnCount
        : undefined,
    title: doc.title,
...
function toConversationGuidanceDirective(
  doc: any,
): ConversationGuidanceDirective {
  return {
    id: doc._id,
...
function toKbDocumentLifecycle(doc: any): KbDocumentLifecycle {
  ...
  return {
    ...
    chunkConfig: {
      chunkSizeChars:
        doc.chunkSizeChars ?? DEFAULT_KB_CHUNK_CONFIG.chunkSizeChars,
      chunkOverlapChars:
        doc.chunkOverlapChars ?? DEFAULT_KB_CHUNK_CONFIG.chunkOverlapChars,
    },
...
class ConvexCouncilRepository implements CouncilRepository {
  ...
  async getThemeMode(): Promise<ThemeMode> {
    const value = await this.client.query(api.settings.getForUser, {
      key: "theme-mode",
    });
  ...
  async setThemeMode(mode: ThemeMode): Promise<void> {
    await this.client.mutation(api.settings.setForUser, {
      key: "theme-mode",
      value: mode,
    });
  }
  ...
  async generateMemberGuidanceProfile(input: {
    ...
    return (await this.client.action(
      api.ai.guidance.generateMemberGuidanceProfile as any,
      {
  ...
  async generateMemberVoicePersona(input: {
    ...
    return (await this.client.action(
      api.ai.voice.generateMemberVoicePersona as any,
      {
        memberId: input.memberId as Id<"members">,
        systemPrompt: input.systemPrompt,
        specialties: input.specialties,
  ...
  async getMemberMemoryBundle(memberId: string): Promise<{
    ...
    return {
      interactionPolicy: result.interactionPolicy
        ? toMemberMemoryDocument(result.interactionPolicy)
        : null,
      mentalModel: result.mentalModel
        ? toMemberMemoryDocument(result.mentalModel)
        : null,
      episodes: (result.episodes ?? []).map(toMemberMemoryEpisode),
      refreshState: result.refreshState
        ? toMemberMemoryRefreshState(result.refreshState)
        : null,
    };
  }

  async saveMemberInteractionPolicy(input: {
    memberId: string;
    body: string;
  }): Promise<MemberMemoryDocument | null> {
    const result = await this.clientAny.mutation(
      "memberMemories:saveInteractionPolicy",
      {
        memberId: input.memberId as Id<"members">,
        body: input.body,
      },
  ...
  async saveMemberMentalModel(input: {
    memberId: string;
    body: string;
  }): Promise<MemberMemoryDocument | null> {
    const result = await this.clientAny.mutation(
      "memberMemories:saveMentalModel",
      {
        memberId: input.memberId as Id<"members">,
        body: input.body,
      },
  ...
  async unlockMemberMemory(input: {
    memberId: string;
  ...
  async queueMemberMemoryRefresh(input: {
    memberId: string;
    force?: boolean;
  ...
  async migrateLegacyProfileNote(): Promise<User> {
    const doc = await this.client.mutation(
      api.users.migrateLegacyProfileNote,
      {},
    );
    return {
      id: doc._id,
  ...
  async listPersonalSourceDocuments(): Promise<PersonalSourceDocument[]> {
    const rows = (await this.client.action(
      api.ai.personalSources.listPersonalSources as any,
      {},
    )) as any[];
  ...
  async createPersonalSourceRecord(input: {
    ...
    const body = (await this.client.action(
      api.ai.personalSources.createPersonalSourceRecord as any,
      {
        stagedFile: {
  ...
  async processPersonalSource(input: {
    ...
    const body = (await this.client.action(
      api.ai.personalSources.processPersonalSource as any,
      {
        personalSourceDocumentId:
          input.personalSourceDocumentId as Id<"personalSourceDocuments">,
      },
    )) as any;
    return {
      ok: Boolean(body.ok),
  ...
  async reprocessPersonalSource(input: {
    ...
    const body = (await this.client.action(
      api.ai.personalSources.reprocessPersonalSource as any,
      {
        personalSourceDocumentId:
          input.personalSourceDocumentId as Id<"personalSourceDocuments">,
        chunkConfig: input.chunkConfig,
  ...
  async deletePersonalSource(input: {
    personalSourceDocumentId: string;
  }): Promise<{ ok: boolean }> {
    return (await this.client.action(
      api.ai.personalSources.deletePersonalSource as any,
      {
        personalSourceDocumentId:
          input.personalSourceDocumentId as Id<"personalSourceDocuments">,
      },
  ...
  async getPersonalSourceDownloadUrl(input: {
    personalSourceDocumentId: string;
  }): Promise<string | null> {
    return (await this.client.query(
      api.personalSourceDocuments.getDownloadUrl as any,
      {
        personalSourceDocumentId:
          input.personalSourceDocumentId as Id<"personalSourceDocuments">,
      },
  ...
  async listPersonalSourceDigests(): Promise<PersonalSourceDigest[]> {
    const rows = (await this.client.query(
      api.personalSourceDigests.listByUser as any,
      {
  ...
  async updatePersonalSourceDigestMetadata(input: {
    ...
    await this.client.mutation(
      api.personalSourceDigests.updateDigestMetadata as any,
      {
  ...
  async listActiveConversationNotebooks(): Promise<ConversationNotebook[]> {
    const docs = await this.clientAny.query(
      "notebooks:listActiveNotebooks",
      {},
    );
    return docs.map(toConversationNotebook);
  }

  async saveConversationNotebook(
    conversationId: string,
    content: string,
  ): Promise<ConversationNotebook | null> {
    const doc = await this.clientAny.mutation(
      "notebooks:upsertNotebookContent",
      {
        conversationId: conversationId as Id<"conversations">,
        content,
      },
  ...
  async listConversations(includeArchived = false): Promise<Conversation[]> {
    const docs = await this.clientAny.query("conversations:list", {
      includeArchived,
    });
  ...
  async listHalls(includeArchived = false): Promise<Conversation[]> {
    const docs = await this.clientAny.query("conversations:listHalls", {
      includeArchived,
    });
    return docs.map(toConversation);
  }
  ...
  async listChambers(includeArchived = false): Promise<Conversation[]> {
    const docs = await this.clientAny.query("conversations:listChambers", {
      includeArchived,
    });
    return docs.map(toConversation);
  }

  async listChamberThreadsByMember(
    memberId: string,
    includeArchived = false,
  ): Promise<Conversation[]> {
    const docs = await this.clientAny.query(
      "conversations:listChambersByMember",
      {
        memberId: memberId as Id<"members">,
        includeArchived,
      },
  ...
  async createChamberThread(memberId: string): Promise<Conversation> {
    const doc = await this.clientAny.mutation(
      "conversations:createChamberThread",
      {
        memberId: memberId as Id<"members">,
      },
    );
  ...
  async startHallFollowUpThread(input: {
    ...
    const result = await this.client.action(
      api.ai.chat.startHallFollowUpThread as any,
      {
  ...
  async getLatestChamberThread(memberId: string): Promise<Conversation | null> {
    const doc = await this.clientAny.query(
      "conversations:getLatestChamberByMember",
      {
        memberId: memberId as Id<"members">,
      },
    );
  ...
  async setChamberResponseMode(
    conversationId: string,
    mode: ChamberResponseMode,
  ): Promise<Conversation> {
    const doc = await this.clientAny.mutation(
      "conversations:setChamberResponseMode",
      {
        conversationId: conversationId as Id<"conversations">,
        mode,
      },
  ...
  async setChamberTimeAwareReentryEnabled(
    conversationId: string,
    enabled: boolean,
  ): Promise<Conversation> {
    const doc = await this.clientAny.mutation(
      "conversations:setChamberTimeAwareReentryEnabled",
      {
        conversationId: conversationId as Id<"conversations">,
        enabled,
      },
  ...
  async setChamberPersonalSourcesEnabled(
    conversationId: string,
    enabled: boolean,
  ): Promise<Conversation> {
    const doc = await this.clientAny.mutation(
      "conversations:setChamberPersonalSourcesEnabled",
      {
        conversationId: conversationId as Id<"conversations">,
        enabled,
      },
  ...
  async setChamberTimeAwareReentryState(input: {
    ...
    const doc = await this.clientAny.mutation(
      "conversations:setChamberTimeAwareReentryState",
      {
        conversationId: input.conversationId as Id<"conversations">,
        state: input.state,
      },
  ...
  async markChamberTimeAwareReentryNoticeSeen(
    conversationId: string,
  ): Promise<Conversation> {
    const doc = await this.clientAny.mutation(
      "conversations:markChamberTimeAwareReentryNoticeSeen",
      {
        conversationId: conversationId as Id<"conversations">,
      },
    );
  ...
  async listConversationGuidanceDirectives(
    conversationId: string,
  ): Promise<ConversationGuidanceDirective[]> {
    const rows = await this.client.query(
      api.guidance.listConversationGuidanceDirectives,
      {
        conversationId: conversationId as Id<"conversations">,
      },
    );
  ...
  async upsertCustomFeedbackGuidance(input: {
    ...
    const doc = await this.clientAny.mutation(
      "guidance:upsertCustomFeedbackGuidance",
      {
        messageId: input.messageId as Id<"messages">,
        chips: input.chips,
        text: input.text,
  ...
  async syncFeedbackGuidanceDirectives(input: {
    ...
    return (await this.client.mutation(
      api.guidance.syncFeedbackGuidanceDirectives,
      {
        messageId: input.messageId as Id<"messages">,
      },
  ...
  async upsertTimeAwareReentryGuidance(input: {
    ...
    return (await this.client.mutation(
      api.guidance.upsertTimeAwareReentryGuidance,
      {
        conversationId: input.conversationId as Id<"conversations">,
  ...
  async reflectChamberGuidance(input: {
    ...
    return (await this.client.action(
      api.ai.guidance.reflectChamberGuidance as any,
      {
        conversationId: input.conversationId as Id<"conversations">,
  ...
  async ensureHallParticipantResponseSlots(
    conversationId: string,
  ): Promise<{ updatedCount: number }> {
    return await this.clientAny.mutation(
      "conversations:ensureHallParticipantResponseSlots",
      {
        conversationId: conversationId as Id<"conversations">,
      },
    );
  ...
  async getLatestChamberMemoryLog(
    conversationId: string,
  ): Promise<ConversationMemoryLog | null> {
    const doc = await this.clientAny.query(
      "memoryLogs:getLatestByConversation",
      {
        conversationId: conversationId as Id<"conversations">,
      },
    );
  ...
  async listMemoryLogsByScope(
    ...
    const docs = await this.clientAny.query(
      "memoryLogs:listByConversationScope",
      {
        conversationId: conversationId as Id<"conversations">,
        scope,
      },
  ...
  async getCompactionPolicy(): Promise<CompactionPolicy> {
    const [thresholdRaw, recentRawTailRaw, hallRawRoundTailRaw] =
      await Promise.all([
        this.client.query(api.settings.get, {
          key: COMPACTION_POLICY_KEYS.threshold,
        }),
        this.client.query(api.settings.get, {
          key: COMPACTION_POLICY_KEYS.recentRawTail,
        }),
        this.client.query(api.settings.get, {
          key: COMPACTION_POLICY_KEYS.hallRawRoundTail,
        }),
      ]);

    return {
      threshold: normalizePolicyNumber(
        thresholdRaw,
        COMPACTION_POLICY_DEFAULTS.threshold,
        1,
      ),
      recentRawTail: normalizePolicyNumber(
        recentRawTailRaw,
        COMPACTION_POLICY_DEFAULTS.recentRawTail,
        1,
      ),
      hallRawRoundTail: normalizePolicyNumber(
        hallRawRoundTailRaw,
        COMPACTION_POLICY_DEFAULTS.hallRawRoundTail,
        1,
      ),
    };
  }

  async getMessagePromptTrace(
    messageId: string,
  ...
  async appendMessages(input: AppendMessagesInput): Promise<Message[]> {
    ...
    const rows = await this.client.mutation(api.messages.appendMany, {
      messages: input.messages.map((message) => ({
        ...
        supersededByMessageId: message.supersededByMessageId as
          | Id<"messages">
          | undefined,
        supersedesMessageId: message.supersedesMessageId as
          | Id<"messages">
          | undefined,
        ...
        routing: message.routing
          ...
          : undefined,
        inReplyToMessageId: message.inReplyToMessageId as
          | Id<"messages">
          | undefined,
        originConversationId: message.originConversationId as
          | Id<"conversations">
          | undefined,
        originMessageId: message.originMessageId as Id<"messages"> | undefined,
        mentionedMemberIds: message.mentionedMemberIds as
          | Id<"members">[]
          | undefined,
        roundNumber: message.roundNumber,
        roundIntent: message.roundIntent,
        roundTargetMemberId: message.roundTargetMemberId as
          | Id<"members">
          | undefined,
        error: message.error,
  ...
  async deleteLatestTurn(input: {
    ...
    return await this.clientAny.mutation("messages:deleteLatestTurn", {
      conversationId: input.conversationId as Id<"conversations">,
      expectedLatestUserMessageId: input.expectedLatestUserMessageId as
        | Id<"messages">
        | undefined,
    });
  }
    ...
    const result = await this.clientAny.mutation(
      "messages:replaceWithRefinement",
      {
        ...
        replacement: {
          ...
          supersededByMessageId: input.replacement.supersededByMessageId as
            | Id<"messages">
            | undefined,
          supersedesMessageId: input.replacement.supersedesMessageId as
            | Id<"messages">
            | undefined,
          ...
          routing: input.replacement.routing
            ? {
                memberIds: input.replacement.routing
                  .memberIds as Id<"members">[],
                source: input.replacement.routing.source,
              }
            : undefined,
          inReplyToMessageId: input.replacement.inReplyToMessageId as
            | Id<"messages">
            | undefined,
          originConversationId: input.replacement.originConversationId as
            | Id<"conversations">
            | undefined,
          originMessageId: input.replacement.originMessageId as
            | Id<"messages">
            | undefined,
          mentionedMemberIds: input.replacement.mentionedMemberIds as
            | Id<"members">[]
            | undefined,
          roundNumber: input.replacement.roundNumber,
          roundIntent: input.replacement.roundIntent,
          roundTargetMemberId: input.replacement.roundTargetMemberId as
            | Id<"members">
            | undefined,
  ...
  async appendElaborationReply(input: {
    ...
    const result = await this.clientAny.mutation(
      "messages:appendElaborationReply",
      {
        targetMessageId: input.targetMessageId as Id<"messages">,
        reply: {
          ...
          authorMemberId: input.reply.authorMemberId as
            | Id<"members">
            | undefined,
          ...
          supersededByMessageId: input.reply.supersededByMessageId as
            | Id<"messages">
            | undefined,
          supersedesMessageId: input.reply.supersedesMessageId as
            | Id<"messages">
            | undefined,
          ...
          routing: input.reply.routing
            ...
            : undefined,
          inReplyToMessageId: input.reply.inReplyToMessageId as
            | Id<"messages">
            | undefined,
          originConversationId: input.reply.originConversationId as
            | Id<"conversations">
            | undefined,
          originMessageId: input.reply.originMessageId as
            | Id<"messages">
            | undefined,
          mentionedMemberIds: input.reply.mentionedMemberIds as
            | Id<"members">[]
            | undefined,
          roundNumber: input.reply.roundNumber,
          roundIntent: input.reply.roundIntent,
          roundTargetMemberId: input.reply.roundTargetMemberId as
            | Id<"members">
            | undefined,
  ...
  async transcribeAudioFromStorage(input: {
    ...
    return (await this.client.action(
      api.ai.voice.transcribeAudioFromStorage as any,
      {
  ...
  async synthesizeMessageSpeech(input: {
    ...
    return (await this.client.action(
      api.ai.voice.synthesizeMessageSpeech as any,
      {
        conversationId: input.conversationId as Id<"conversations">,
        messageId: input.messageId as Id<"messages">,
      },
  ...
  async suggestChamberTitle(input: {
    message: string;
    model?: string;
  }): Promise<HallTitleResult> {
  ...
  async suggestMemberSpecialties(input: {
    ...
    return (await this.client.action(
      api.ai.routing.suggestMemberSpecialties as any,
      {
        name: input.name,
        systemPrompt: input.systemPrompt,
        model: input.model,
  ...
  async chatWithMember(input: {
    ...
    return (await this.client.action(api.ai.chat.chatWithMember as any, {
      ...
      chatProfile:
        input.chatProfile === "brainstorm" ? "instant" : input.chatProfile,
      retrievalStrategy: input.retrievalStrategy,
      retrievalProfile:
        input.retrievalStrategy === "deep_dive" ? "deep_dive" : "default",
  ...
  async prepareRoundtableRound(input: {
    ...
    const result = await this.client.action(
      api.ai.roundtable.prepareRoundtableRound as any,
      {
        conversationId: input.conversationId as Id<"conversations">,
        trigger: input.trigger,
        triggerMessageId: input.triggerMessageId as Id<"messages"> | undefined,
        mentionedMemberIds: input.mentionedMemberIds as
          | Id<"members">[]
          | undefined,
      },
    );
  ...
  async refreshRoundtableRound(input: {
    ...
    const result = await this.client.action(
      api.ai.roundtable.refreshRoundtableRound as any,
      {
        conversationId: input.conversationId as Id<"conversations">,
        roundNumber: input.roundNumber,
      },
  ...
  async markRoundtableInProgress(input: {
    ...
    const result = await this.clientAny.mutation(
      "hallRounds:markRoundInProgress",
      {
        conversationId: input.conversationId as Id<"conversations">,
        roundNumber: input.roundNumber,
  ...
  > {
    return (await this.client.action(
      api.ai.roundtable.chatRoundtableSpeaker as any,
      {
        conversationId: input.conversationId as Id<"conversations">,
        roundNumber: input.roundNumber,
        memberId: input.memberId as Id<"members">,
  ...
  > {
    const result = await this.client.action(
      api.ai.roundtable.chatRoundtableSpeakers as any,
      {
        conversationId: input.conversationId as Id<"conversations">,
        roundNumber: input.roundNumber,
        debugPromptTrace: input.debugPromptTrace,
  ...
  async ensureMemberStore(input: {
    memberId: string;
  }): Promise<{ storeName: string; created: boolean }> {
    return (await this.client.action(
      api.ai.knowledge.ensureMemberKnowledgeStore as any,
      {
        memberId: input.memberId as Id<"members">,
      },
  ...
  async createKbDocumentRecord(input: {
    ...
    const body = (await this.client.action(
      api.ai.knowledge.createKbDocumentRecord as any,
      {
        memberId: input.memberId as Id<"members">,
        stagedFile: {
          storageId: input.stagedFile.storageId as Id<"_storage">,
  ...
  async startKbDocumentProcessing(input: {
    kbDocumentId: string;
  }): Promise<{ ok: boolean; document: KbDocumentLifecycle }> {
    const body = (await this.client.action(
      api.ai.knowledge.startKbDocumentProcessing as any,
      {
  ...
  async retryKbDocumentIndexing(input: {
    kbDocumentId: string;
  }): Promise<{ ok: boolean; document: KbDocumentLifecycle }> {
    const body = (await this.client.action(
      api.ai.knowledge.retryKbDocumentIndexing as any,
      {
        kbDocumentId: input.kbDocumentId as Id<"kbDocuments">,
      },
    )) as any;
  ...
  async retryKbDocumentMetadata(input: {
    kbDocumentId: string;
  }): Promise<{ ok: boolean; document: KbDocumentLifecycle }> {
    const body = (await this.client.action(
      api.ai.knowledge.retryKbDocumentMetadata as any,
      {
        kbDocumentId: input.kbDocumentId as Id<"kbDocuments">,
      },
    )) as any;
  ...
  async reprocessKbDocument(input: {
    ...
    const body = (await this.client.action(
      api.ai.knowledge.reprocessKbDocument as any,
      {
        kbDocumentId: input.kbDocumentId as Id<"kbDocuments">,
        chunkConfig: input.chunkConfig,
      },
  ...
  async getKbDocumentDownloadUrl(input: {
    kbDocumentId: string;
  }): Promise<string | null> {
  ...
  async listKbDocuments(input: {
    memberId: string;
  }): Promise<KbDocumentLifecycle[]> {
    const rows = (await this.client.action(
      api.ai.knowledge.listKbDocumentsByMember as any,
      {
        memberId: input.memberId as Id<"members">,
      },
    )) as any[];
  ...
  async uploadMemberDocuments(input: {
    ...
    return (await this.client.action(
      api.ai.knowledge.uploadMemberDocuments as any,
      {
        memberId: input.memberId as Id<"members">,
  ...
  async listMemberDocuments(input: {
    memberId: string;
  }): Promise<Array<{ name?: string; displayName?: string }>> {
    return (await this.client.action(
      api.ai.knowledge.listMemberKnowledgeDocuments as any,
      {
        memberId: input.memberId as Id<"members">,
      },
  ...
  async deleteMemberDocument(input: {
    ...
    return (await this.client.action(
      api.ai.knowledge.deleteMemberKnowledgeDocument as any,
      {
        memberId: input.memberId as Id<"members">,
  ...
  async listMemberDigestMetadata(input: {
    memberId: string;
  ...
  async rehydrateMemberStore(input: {
    ...
    return (await this.client.action(
      api.ai.knowledge.rehydrateMemberKnowledgeStore as any,
      {
        memberId: input.memberId as Id<"members">,
        mode: input.mode,
      },
  ...
  async purgeExpiredStagedDocuments(input: {
    memberId?: string;
  }): Promise<{ purgedCount: number }> {
    return (await this.client.action(
      api.ai.knowledge.purgeExpiredStagedKnowledgeDocuments as any,
      {
        memberId: input.memberId
          ? (input.memberId as Id<"members">)
          : undefined,
      },
