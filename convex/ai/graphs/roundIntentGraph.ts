  ...
  const graph = new StateGraph(RoundIntentStateAnnotation)
    .addNode('propose', async (state) => {
      ...
      const prompt = [
        ...
        'Prefer speaking when the user has materially pushed, reframed, or sharpened the question.',
        'Choose pass only when you would mostly repeat what is already on the table.',
        'A meaningful delta means a real rebuttal, refinement, qualification, new evidence, or genuine shift in view.',
        'Do not volunteer just to restate your opener or repeat another member in different words.',
        'It is good to keep listening even when you disagree.',
        '',
